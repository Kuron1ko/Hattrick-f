...
class DM_Dataset_within_Cluster(Dataset):
    def __init__(self, props, cluster, start, end):
        ...
        self.loaded_index_range = (int(start), int(end))
        self.max_source_index_read = int(end) - 1
        ...
        self.results_path = str(
            Path(
                getattr(
                    props,
                    "baseline_dir",
                    self.workspace_root / "baseresult" / dataset_name,
                )
            ).resolve()
        )
        count = end - start
        filenames = np.loadtxt(
            f"{self.results_path}/filenames.txt",
            dtype="U",
            delimiter=",",
            skiprows=start,
            max_rows=count,
        ...
        if self.props.failure_id == None:
            ...
            opts1_mf = np.loadtxt(
                f"{self.results_path}/gt_optimal_values_mf.txt",
                dtype=np.float32,
                skiprows=start,
                max_rows=count,
            ).ravel()
            ...
            opts2_mf = np.loadtxt(
                f"{self.results_path}/gt_optimal_values_mf_mf.txt",
                dtype=np.float32,
                skiprows=start,
                max_rows=count,
            ).ravel()
            ...
            opts3_mf = np.loadtxt(
                f"{self.results_path}/gt_optimal_values_mf_mf_mf.txt",
                dtype=np.float32,
                skiprows=start,
                max_rows=count,
            ).ravel()
            ...
            opts1 = np.loadtxt(
                f"{self.results_path}/gt_optimal_values_mlu.txt",
                dtype=np.float32,
                skiprows=start,
                max_rows=count,
            ).ravel()
            ...
            opts2 = np.loadtxt(
                f"{self.results_path}/gt_optimal_values_mlu_mlu.txt",
                dtype=np.float32,
                skiprows=start,
                max_rows=count,
            ).ravel()
            ...
            if props.rate_cap:
                opts3 = np.loadtxt(
                    f"{self.results_path}/gt_optimal_values_mf_3.txt",
                    dtype=np.float32,
                    skiprows=start,
                    max_rows=count,
                ).ravel()
                ...
                opts3 = np.loadtxt(
                    f"{self.results_path}/gt_optimal_values_mlu_mlu_mlu.txt",
                    dtype=np.float32,
                    skiprows=start,
                    max_rows=count,
                ).ravel()
        ...
        self.path_masks = cluster_info.get_path_masks() if self.props.path_mask else None
