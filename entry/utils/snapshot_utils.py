...
class Read_Snapshot:
    def __init__(self, props, topology_filename, pairs_filename, tm_filename):
        ...
        dataset_name = str(
            getattr(
                props,
                "dataset_name",
                "1x" if "load1x" in self.topo else "3x" if "load3x" in self.topo else "2x",
            )
        )
        self.data_dir = Path(
            getattr(props, "data_dir", self.workspace_root / "data" / dataset_name)
        ).resolve()
    ...
    def read_pairs_from_pkl(self, pairs_filename):
        file = open(self.data_dir / "pairs" / pairs_filename, "rb")
    ...
    def write_graph_to_json(self, filename, graph, priority=3):
        ...
        path = self.data_dir / f"topology_{priority}" / filename
    ...
    def read_graph_from_json_c2(self, topo: str, tm_filename):
        ...
        with open(self.data_dir / "topology_2" / topology_filename) as f:
            data = json.load(f)
    ...
    def read_graph_from_json(self, topo: str, topology_filename):
        with open(self.data_dir / "topology" / topology_filename) as f:
            data = json.load(f)
    ...
    def read_tms(self, tm_filename, priority):
        file = open(self.data_dir / "traffic" / str(priority) / tm_filename, 'rb')
        tm = pickle.load(file)
        tm = tm.reshape(-1, 1)
    ...
    def read_tms_pred(self, tm_filename, priority, pred_type):
        file = open(
            self.data_dir / "traffic" / f"{priority}_{pred_type}" / tm_filename,
            'rb',
        )
        tm = pickle.load(file)
        tm = tm.reshape(-1, 1)
