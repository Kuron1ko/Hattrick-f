...
class Cluster_Info:
    def __init__(self, sp: Read_Snapshot, props, cluster):
        ...
        dataset_name = str(
            getattr(
                props,
                "dataset_name",
                "1x" if "load1x" in props.topo else "3x" if "load3x" in props.topo else "2x",
            )
        )
        workspace_root = Path(
            getattr(props, "workspace_root", self.parent_dir_path)
        ).resolve()
        self.path_cache_dir = Path(
            getattr(
                props,
                "path_cache_dir",
                workspace_root / "data" / dataset_name / "path_cache",
            )
        ).resolve()
    ...
    def compute_ksp_paths(self, k, pairs):
        ...
        filepath = self.path_cache_dir / f"{self.props.topo}_{k}_paths_dict_cluster_{self.cluster}.pkl"
        try:
    ...
    def get_padded_edge_ids_per_path(self, pij, edges_map) -> torch.Tensor:
        ...
        """
        filepath = self.path_cache_dir / f"{self.props.topo}_{self.props.num_paths_per_pair}_paths_cluster_{self.cluster}_padded_edge_ids_per_path.pkl"
        filepath_1 = self.path_cache_dir / f"{self.props.topo}_{self.props.num_paths_per_pair}_paths_cluster_{self.cluster}_edge_ids_dict.pkl"
        try:
    ...
    def get_paths_to_edges_matrix(self, pij: dict) -> torch.sparse_coo_tensor:
        ...
        filepath = self.path_cache_dir / f"{self.props.topo}_{self.props.num_paths_per_pair}_paths_cluster_{self.cluster}.pkl"
        try:
    ...
    def get_path_masks(self) -> torch.Tensor:
        filepath = self.path_cache_dir / f"{self.props.topo}_{self.props.num_paths_per_pair}_path_masks_cluster_{self.cluster}.pkl"
        with open(filepath, "rb") as file:
            masks = pickle.load(file)
        masks = torch.tensor(masks, dtype=torch.bool)
        expected_shape = (3, self.num_pairs, self.props.num_paths_per_pair)
        if tuple(masks.shape) != expected_shape:
            raise ValueError(f"{filepath} has shape {tuple(masks.shape)}, expected {expected_shape}")
        if not torch.all(masks.any(dim=-1)):
            raise ValueError(f"{filepath} contains a class/pair with no allowed path")
        return masks.reshape(3, -1)
