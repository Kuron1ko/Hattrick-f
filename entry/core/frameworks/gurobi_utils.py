...
class GurobiModel: 
    def __init__(self, props: argparse.ArgumentParser, current_sp: Read_Snapshot, mode="flexile"):
        ...
        self.mode = mode
        self.path_masks = self.load_path_masks() if self.props.path_mask else None
        if self.props.pred:
    ...
    def load_path_masks(self):
        filepath = f"{parent_dir}/topologies/path_masks/{self.props.topo}_{self.props.num_paths_per_pair}_path_masks_cluster_{self.props.cluster}.pkl"
        with open(filepath, "rb") as file:
            masks = pickle.load(file)
        masks = np.asarray(masks, dtype=bool)
        expected_shape = (3, self.current_sp.num_demands, self.props.num_paths_per_pair)
        if masks.shape != expected_shape:
            raise ValueError(f"{filepath} has shape {masks.shape}, expected {expected_shape}")
        if not masks.any(axis=2).all():
            raise ValueError(f"{filepath} contains a class/pair with no allowed path")
        return masks

    def variable_upper_bounds(self, class_index):
        if self.path_masks is None:
            return 1.0
        return self.path_masks[class_index].astype(np.float64)
        ...
        if self.mode == "flexile":
            for i in range(len(objs)):
                self.model_variables[i] = self.model.addMVar((self.current_sp.num_demands,
                                        ...
                                        lb=0.0, ub=self.variable_upper_bounds(i))
        elif self.mode == "swan":
            self.model_variables[self.props.priority - 1] = self.model.addMVar((self.current_sp.num_demands,
                                        self.props.num_paths_per_pair),
                                        vtype=GRB.CONTINUOUS,
                                        lb=0.0, ub=self.variable_upper_bounds(self.props.priority - 1))
