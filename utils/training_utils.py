...
def train(epoch, n_epochs, model, props, ds_list, dl_list, optimizers):
    ...
    for j in range(len(ds_list)):
        ...
        path_masks = train_dataset.path_masks.to(props.device) if train_dataset.path_masks is not None else None
        model.train()
            ...
            for i, inputs in enumerate(tepoch):
                ...
                if props.pred:
                    ...
                    output = model(props, node_features, train_dataset.edge_index, capacities,
                        ...
                        train_dataset.edge_ids_dict_tensor, train_dataset.original_pos_edge_ids_dict_tensor, path_masks)
                    end = time.time()
                ...
                start = time.time()
                use_path_cost = (
                    getattr(props, "edge_cost_objective", False)
                    and getattr(props, "edge_cost_mode", "linear")
                    not in {
                        "weighted_mlu",
                        "weighted_mlu_actual",
                ...
                if use_path_cost:
                    loss1, loss1_val = loss_path_cost(edges_util_tm1_1)
                    loss2, loss2_val = loss_path_cost(edges_util_tm2_2)
                else:
                    loss1, loss1_val = loss_mlu(edges_util_tm1_1, opt1)
                    loss2, loss2_val = loss_mlu_overflow_area(
                        edges_util_tm2_2,
                        opt2,
                        float(getattr(props, "hm_overflow_lambda", 0.0)),
                    )
                loss3, loss3_val = loss_mf(all_traffic, opt3_mf)
                if use_path_cost:
                    loss4, loss4_val = loss_path_cost(edges_util_tm3_3)
                else:
...
def loss_mlu_overflow_area(y_pred_batch, y_true_batch, overflow_lambda=0.0):
    """Native MLU plus a High+Medium multi-bottleneck tie-break.

    The native term and reported validation metric are untouched.  The added
    training-only term gives every edge above physical capacity a gradient,
    instead of giving a gradient only to the single maximum-utilization edge.
    Returning early at lambda zero preserves exact native outputs and gradients.
    """
    native_loss, native_value = loss_mlu(y_pred_batch, y_true_batch)
    if overflow_lambda == 0.0:
        return native_loss, native_value
    overflow_area = torch.relu(y_pred_batch - 1.0).square()
    overflow_area = overflow_area.reshape(overflow_area.shape[0], -1).mean(dim=1)
    return native_loss + overflow_lambda * overflow_area.mean(), native_value


def loss_path_cost(cost_batch):
    """Minimize sum(path flow * learned edge price), normalized per sample.

    ``Hattrick.forward`` supplies one scalar for each of the three cumulative
    priority stages.  Detaching only the denominator keeps objective scales
    comparable for projected-gradient training without changing the direction.
    """
    flat_cost = cost_batch.reshape(cost_batch.shape[0], -1).sum(dim=1)
    denominator = flat_cost.detach().clamp_min(1e-12)
    loss = (flat_cost / denominator).mean()
    return loss, float(flat_cost.detach().mean().item())
...
def validate(model, props, val_ds_list, val_dl_list):
        ...
        for k in range(len(val_ds_list)):
            ...
            path_masks = val_dataset.path_masks.to(props.device) if val_dataset.path_masks is not None else None
            
            val_norm_mlu = _validate(model, props, val_dataset, val_dl, path_masks)
...
def _validate(model, props, val_ds, val_dl, path_masks=None):
    val_norm_mlu1 = []
    val_norm_mlu2 = []
    val_norm_mlu3 = []
    all_traffic_list = []
            ...
            for i, inputs in enumerate(vals):
                ...
                if props.pred:
                    ...
                    output = model(props, node_features, val_ds.edge_index, capacities,
                            ...
                            val_ds.edge_ids_dict_tensor, val_ds.original_pos_edge_ids_dict_tensor, path_masks)
                # If prediction is off, feed the actual matrix as predicted matrix
                else:
                    output = model(props, node_features, val_ds.edge_index, capacities,
                            val_ds.padded_edge_ids_per_path,
