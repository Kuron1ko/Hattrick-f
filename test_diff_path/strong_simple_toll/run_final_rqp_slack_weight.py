...
adaptive = load_module(
    "final_rqp_slack_weight_base", HERE / "probe_rqp_slack_weight.py"
)
final, base = adaptive.final, adaptive.base

SPEC = {"threshold": 0.90, "low_floor": -0.005}
...
def benchmark(model, props, cache, factors, repetitions):
    adaptive.construct(model, props, cache, factors, SPEC)
    synchronize()
...
def main():
    ...
    report = {
        "method": "RQP-S: single-stage ESM High-slack adaptive weighting",
        ...
        "config": {
            ...
            "high_weight_base": adaptive.HIGH_BASE,
            "high_weight_transfer": adaptive.HIGH_TRANSFER,
            "medium_weight_max": adaptive.MEDIUM_MAX,
            "high_slack_threshold": SPEC["threshold"],
            "high_slack_temperature": adaptive.TEMPERATURE,
            "low_predicted_floor": SPEC["low_floor"],
            "optimization_steps": final.STEPS,
        },
        "loads": {},
    }
    for load in (1, 2, 3):
        ...
        candidate, active, medium_gate = adaptive.construct(
        ...
        report["loads"][str(load)] = {
            ...
            "medium_gate": {
                "mean": float(medium_gate.mean()),
                "p10": float(torch.quantile(medium_gate, 0.10)),
                "p90": float(torch.quantile(medium_gate, 0.90)),
            },
            "evaluation": {
        ...
        print(
            ...
            f"gate={float(medium_gate.mean()):.3f} "
    ...
    path = HERE / "final_rqp_slack_weight.json"
