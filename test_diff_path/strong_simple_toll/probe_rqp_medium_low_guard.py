...
probe = load_module("rqp_medium_low_guard_base", HERE / "probe_rqp_medium.py")
base = probe.base
...
def main():
    ...
    specs = [
        ...
        for high_weight, medium_weight in pairs
        for low_floor in (-0.01, -0.005, 0.0)
    ]
    baselines, ranked = probe.run_bounds(
        (2, 3), (334, 400), specs, "guard-validation"
    )
    ranked.sort(key=constrained_score, reverse=True)
    for value in ranked:
        value["constrained_score"] = constrained_score(value)
    path = HERE / "rqp_medium_low_guard.json"
    path.write_text(
        json.dumps(
            {
                "method": "RQP-M with one predicted Low non-regression guard",
