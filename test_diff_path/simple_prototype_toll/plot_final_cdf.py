...
REPORT = HERE / "final_level4_report.json"
LOADS = ("1", "2", "3")
CLASSES = ("High", "Medium", "Low")
SERIES = (("Hattrick", "baseline"), ("P3-CFT · strict ESM", "candidate"))
COLORS = {"Hattrick": "#2F78D4", "P3-CFT · strict ESM": "#F28E2B"}
LINESTYLES = {"Hattrick": "-", "P3-CFT · strict ESM": "--"}
AXES = {
...
def values(payload: dict, load: str, source: str, class_name: str) -> np.ndarray:
    ...
    if len(selected) != 100:
        raise ValueError(f"Expected 100 rows for {load}x/{source}/{class_name}")
    return np.asarray(selected, dtype=np.float64)


def shared_bandwidth(series: list[np.ndarray], left: float, right: float) -> float:
...
def smooth_cdf(samples: np.ndarray, left: float, right: float, bandwidth: float):
    span = right - left
...
def main() -> None:
    ...
    figure.text(
        0.055,
        0.965,
        "Load-specific Hattrick versus three-prototype congestion-fallback tolls · Level-4 snapshots 400–499",
        ha="left",
        color="#596574",
        fontsize=12.5,
    )
    ...
    figure.savefig(HERE / "final_p3_cft_cdf.png", dpi=210, bbox_inches="tight")
    figure.savefig(HERE / "final_p3_cft_cdf.pdf", bbox_inches="tight")
    plt.close(figure)
