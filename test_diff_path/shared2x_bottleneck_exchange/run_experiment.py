...
BASE_EPSILON_DIR = TEST_DIR / "shared2x_order_epsilon"
...
BASE_ARTIFACTS = BASE_EPSILON_DIR / "artifacts"
...
def phase_a_directory(level: int, seed: int) -> Path:
    # Reuse the frozen restored-Fh/Fhm checkpoints from the preceding epsilon
    # experiment.  New Phase-B arms never modify this external directory.
    return BASE_ARTIFACTS / "phase_a" / LEVELS[level]["label"] / f"seed_{seed}"


def multiplier_label(multiplier: float) -> str:
    return format(float(multiplier), ".12g").replace(".", "p")
...
def run_directory(
    ...
    if approach in ("bottleneck", "bottleneck_order"):
        root = root / f"multiplier_{multiplier_label(bottleneck_multiplier)}"
...
def train_epoch(
    ...
    *,
    ...
    for batch_index, inputs in enumerate(loader):
        ...
        components = forward_components(model, props, dataset, values, path_masks)
        ...
        if approach in (
            ...
            if reference_model is None:
                raise RuntimeError("transfer approach requires a frozen Phase-A model")
            reference_model.eval()
            with torch.no_grad():
                reference = forward_components(
                    reference_model, props, dataset, values, path_masks
                )
            reference_medium = reference["admitted_medium"]
            reference_low = reference["admitted_low"]
        curriculum_scale = 0.0 if epoch <= max(1, total_epochs // 2) else 1.0
        losses, reported, names, guard, auxiliary = build_objectives(
            approach,
            epsilon,
            **components,
        ...
        if guard is not None:
            ...
            totals["high_tail_fraction"] = totals.get("high_tail_fraction", 0.0) + float(
                guard.tail_fraction
            )
            totals["high_tail_count"] = totals.get("high_tail_count", 0.0) + float(
                guard.tail_count
            )
        for name, value in auxiliary.items():
            if value.numel() == 0:
                continue
            totals[f"aux_{name}_mean"] = totals.get(
                f"aux_{name}_mean", 0.0
            ) + float(value.detach().mean().item())
            if name in ("order_violation", "transfer_deficit"):
                totals[f"aux_{name}_active_fraction"] = totals.get(
                    f"aux_{name}_active_fraction", 0.0
                ) + float((value.detach() > 0).to(dtype=value.dtype).mean().item())
        totals["curriculum_scale"] = totals.get("curriculum_scale", 0.0) + curriculum_scale
        count += 1
    props.research_return_admitted = False
...
def ensure_phase_a(level: int, seed: int, *, force: bool = False) -> Path:
    ...
    raise FileNotFoundError(
        f"frozen Phase-A checkpoint is missing; this experiment will not create or modify it: {run_dir}"
    )
    # The legacy training body below is intentionally unreachable.  It remains
    # available as provenance for how the imported checkpoint was produced.
    if force:
        safe_remove(run_dir)
...
def checkpoint_rank(
    ...
    if approach in (
        ...
        medium_gain = (
            float(indexed["Medium"]["admitted_traffic_mean"])
            - float(initial["Medium"]["admitted_traffic_mean"])
        )
        low_loss = max(
            0.0,
            float(initial["Low"]["admitted_traffic_mean"])
            - float(indexed["Low"]["admitted_traffic_mean"]),
        )
        transfer_safe = medium_gain + 1e-8 >= 0.99 * low_loss
    return (
        int(safe and high_safe and transfer_safe),
...
def phase_b_config(
    ...
    names = {
        ...
        "tail": ("HighTail", "Fm", "Uhm", "Fhml", "Uhml", "Uh"),
        "tail_order": (
            "HighTail", "Order", "Fm", "Uhm", "Fhml", "Uhml", "Uh"
        ),
        "tail_order_transfer": (
            "HighTail", "Order", "Transfer", "Fm", "Uhm",
            "FhmlCurriculum", "UhmlCurriculum", "Uh",
        ),
        "tail_order_transfer_normalized": (
            "HighTail", "Order", "TransferNorm", "Fm", "Uhm",
            "FhmlCurriculum", "UhmlCurriculum", "Uh",
        ),
        "bottleneck": (
        ...
        "bottleneck_order": (
            "HighTail", "TransferNorm", "Order", "BottleneckRelease", "Fm", "Uhm",
            "FhmlCurriculum", "UhmlCurriculum", "Uh",
        ),
    }[approach]
    return {
        ...
        "high_tail_definition": (
            "mean of largest 20% per-snapshot ReLU(1-epsilon/2-N_high); hard gate at 1-epsilon-1e-4"
            if approach != "control"
            else None
        ),
        "order_definition": (
            "mean ReLU(N_low-N_medium), with gradients through both classes"
            if approach in (
                "tail_order",
                "tail_order_transfer",
                "tail_order_transfer_normalized",
        ...
        "transfer_definition": (
            "mean ReLU(positive Phase-A Low loss - signed Medium delta)"
            if approach in (
                "tail_order_transfer",
                "tail_order_transfer_normalized",
                "bottleneck",
        ...
        "curriculum": (
            "Fhml/Uhml disabled for first half of Phase-B epochs"
            if approach in (
                "tail_order_transfer",
                "tail_order_transfer_normalized",
                "bottleneck",
        ...
        "bottleneck_definition": (
            "detached inversion gate times Low admitted utilization on detached High+Medium links above 85%; direct gradient only through Low, Medium optimized by Fm"
            if approach in ("bottleneck", "bottleneck_order")
            else None
        ),
        "checkpoint_low_policy": (
            "Low is not protected by a percentage guard. For transfer/bottleneck arms, "
            "validation selection requires Medium admitted gain >= 0.99 times Low admitted loss "
            "relative to the shared Phase-A checkpoint."
        ),
        "checkpoint_rank": (
...
def run_phase_b(
    ...
    *,
    ...
    run_dir = run_directory(
        level, approach, epsilon, seed, bottleneck_multiplier
    )
    complete_path = run_dir / "complete.json"
    ...
    if approach in (
        ...
        reference_model = Hattrick(props).to(device=device, dtype=props.dtype)
        reference_model.load_state_dict(phase_a["model_state_dict"])
        reference_model.eval()
    ...
    for epoch in range(start_epoch, int(LEVELS[level]["phase_b_epochs"]) + 1):
        ...
        train_metrics = train_epoch(
            ...
            epsilon,
            epoch=epoch,
            total_epochs=int(LEVELS[level]["phase_b_epochs"]),
    ...
    for checkpoint_name, checkpoint_path in (("final", final_path), ("best", best_path)):
        ...
        gate = high_gate_stats(rows, float(epsilon)) if approach != "control" else None
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Shared-2x High-tail, Medium/Low order, and transfer experiment"
    )
    ...
    parser.add_argument("--bottleneck-multiplier", type=float, default=1.0)
    parser.add_argument("--seed", type=int, default=490)
    parser.add_argument("--force", action="store_true")
    args = parser.parse_args()
    if args.approach != "control" and args.epsilon is None:
        parser.error("tail approaches require --epsilon 0.005")
    if args.approach == "control" and args.epsilon is not None:
        parser.error("--epsilon is only valid with tail approaches")
    if (
        args.approach not in ("bottleneck", "bottleneck_order")
        and args.bottleneck_multiplier != 1.0
    ):
        parser.error("--bottleneck-multiplier is only valid with bottleneck approaches")
    if not math.isfinite(args.bottleneck_multiplier) or args.bottleneck_multiplier <= 0:
        parser.error("--bottleneck-multiplier must be finite and positive")
    if args.level == 0:
        ...
        source = BASE_ARTIFACTS / "level0_lp" / "complete.json"
        result = json.loads(source.read_text(encoding="utf-8"))
        print(json.dumps(result, indent=2), flush=True)
        return
    if args.level >= 2:
        level0 = BASE_ARTIFACTS / "level0_lp" / "complete.json"
    ...
    ensure_phase_a(args.level, args.seed, force=False)
    path = run_phase_b(
        args.level,
        args.approach,
        args.epsilon,
