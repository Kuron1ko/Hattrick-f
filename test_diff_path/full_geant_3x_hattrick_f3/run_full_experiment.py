...
"""Train Hattrick-f3 on the complete GEANT 3x workspace.

The algorithm is copied from the complete 2x Hattrick-f3 runner: Phase B
blends the six-loss ordered update into the flow-only update with a 12-epoch
cosine release of MLU.  Data, Oracle, strict-ESM evaluation, and the Phase-A
checkpoint come exclusively from the registered 3x workspace.
"""
...
BASE_3X_PATH = TEST_DIR / "full_geant_3x_hattrick" / "run_full_experiment.py"
ALGORITHM_PATH = TEST_DIR / "full_geant_2x_hattrick_f3" / "run_full_experiment.py"
DEFAULT_PHASE_A = (
    TEST_DIR / "full_geant_3x_hattrick" / "artifacts" / "hattrick"
...
base3 = load_module("full_geant_3x_hattrick_runtime", BASE_3X_PATH)
algorithm = load_module("full_geant_3x_f3_algorithm", ALGORITHM_PATH)
base = base3.base
...
def source_hashes() -> dict[str, str]:
    paths = {
        "full_geant_3x_hattrick_f3.py": Path(__file__).resolve(),
        "full_geant_3x_hattrick.py": BASE_3X_PATH,
...
def experiment_config(
    ...
    return {
        ...
        "seed": args.seed,
        "topology": base3.TARGET_TOPOLOGY,
        "load_factor": base3.LOAD_FACTOR,
        "strict_esm": True,
...
def resume_config_compatible(saved: dict, current: dict) -> bool:
    """Allow only checkpoint-policy upgrades and/or a larger epoch target."""
...
def check_inputs(
    ...
    *,
    ...
    base3.require_training_inputs()
    phase_a = (phase_a_override or args.phase_a_checkpoint).resolve()
    if not phase_a.exists():
        raise FileNotFoundError(f"Missing 3x Phase-A Hattrick checkpoint: {phase_a}")
    ...
    if checkpoint.get("dataset") != DATASET_NAME:
        raise RuntimeError(f"Phase-A checkpoint is not a 3x model: {phase_a}")
    if int(checkpoint.get("seed", -1)) != args.seed:
        raise RuntimeError("Phase-A checkpoint seed mismatch")
    config = checkpoint.get("config", {})
    if (
        config.get("topology") != base3.TARGET_TOPOLOGY
        or float(config.get("load_factor", -1)) != base3.LOAD_FACTOR
    ):
    ...
    if require_selection_index:
        entries = base3.load_top_k(phase_a.parent)
        if not entries:
...
def train(args: argparse.Namespace) -> None:
    ...
    if config_path.exists():
        ...
        if saved_config == current_config:
            ...
            raise RuntimeError(f"Existing 3x Hattrick-f3 configuration differs: {config_path}")
    else:
        config = current_config
    ...
    device = base3.resolve_device(args.device)
    ...
    top_entries = base3.load_top_k(run_dir)
    start_epoch = 1
    if args.resume:
        ...
        print(f"[resume] 3x Hattrick-f3 at epoch {start_epoch}", flush=True)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        archive = base3.save_epoch_checkpoint(run_dir, payload)
        top_entries = base3.update_top_k(run_dir, top_entries, payload, args.top_k)
        print(
            f"[3x Hattrick-f3] epoch={epoch}/{args.epochs} alpha={alpha:.4f} "
    ...
    complete = {
        ...
        "saved_epoch_checkpoints": base3.saved_epoch_numbers(run_dir),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train full-GEANT 3x Hattrick-f3")
